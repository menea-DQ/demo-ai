...
const sign = (data: string) => createHmac("sha256", sec.sessionSecret).update(data).digest("base64url");
...
/** Crea un nuovo session-id e il relativo token firmato. */
...
export function verifySessionToken(token: string | undefined | null): string | null {
  ...
  try {
    ...
    if (Math.floor(Date.now() / 1000) - payload.iat > sec.sessionTtlSec) return null;
    return payload.sid;
  } catch {
    return null;
  }
